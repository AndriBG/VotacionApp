<?php
 ...
 class ServiceDataBaseResultado{
    ...
    public function Add($item){
        ...
        $stmt = $this->context->db->prepare("insert into resultado (EleccionId,PuestoId,CandidatoId,Votos,Porcentaje) values(?,?,?,?,?)");
        $stmt->bind_param("iiiid", $item->Id, $item->Nombre,$item->Apellido,$item->Email,$item->Estado);
        $stmt->execute();
        $stmt->close();
    }
